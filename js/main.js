...
// --- INÍCIO DA LÓGICA DE ASSUNTOS ---
const flatSubjects = flattenTreeWithObjects(subjectTree);
// --- FIM DA LÓGICA DE ASSUNTOS ---
...
// Variáveis para a lógica de Colaboradores
let colaboradores = [];
...
const togglePautaLock = (isClosed) => {
    ...
    const actionButtons = document.querySelectorAll('.check-in-btn, .attend-btn, .faltou-btn, .return-to-pauta-btn, .delete-btn, .priority-btn, .edit-assisted-btn, .edit-attendant-btn, .return-to-aguardando-btn, .manage-demands-btn');
...
const renderAssistedList = () => {
    ...
    aguardando.sort((a, b) => {
        ...
        if (a.priority === 'URGENTE' && b.priority === 'URGENTE') {
...
const loadPauta = async (pautaId, pautaName, pautaType) => {
    ...
    if (pautaDoc.exists()) {
        ...
        if (new Date() > expirationDate) {
            showNotification("Pauta inacessível pois cumpriu seu prazo.", "error");
            return;
        }
        ...
        document.getElementById('order-explanation-title').textContent = ordem === 'padrao' ? 'Entenda a Ordem de Atendimento Padrão' : 'Entenda a Ordem de Chegada';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function showPanel() {
        if(actionsPanel && actionsArrow) {
            actionsPanel.classList.remove('opacity-0', 'scale-90', 'pointer-events-none');
    ...
    function hidePanel() {
        if(actionsPanel && actionsArrow) {
            actionsPanel.classList.add('opacity-0', 'scale-90', 'pointer-events-none');
    ...
    // --- NOVO FLUXO DE CRIAÇÃO DE PAUTA COM ORDEM DE ATENDIMENTO ---
    ...
    document.querySelectorAll('.pauta-type-btn').forEach(button => {
        button.addEventListener('click', (e) => {
            ...
            document.getElementById('pauta-type-modal').classList.add('hidden');
            document.getElementById('create-pauta-modal').dataset.pautaType = type;
                ...
                lines.forEach(line => {
                    const parts = line.split(';').map(item => item.trim().replace(/^"|"$/g, ''));
                     if (parts.length >= 3) {
                         ...
                         const cpf = parts.length > 3 ? parts[3].replace(/^"|"$/g, '') : '';
    ...
    document.getElementById('download-pdf-btn').addEventListener('click', () => {
        ...
        const atendidos = allAssisted
            ...
            .sort((a, b) => new Date(b.attendedTime) - new Date(a.attendedTime));
    ...
    document.body.addEventListener('click', async (e) => {
        ...
        if (id && collectionRef) {
            ...
            if(isPautaClosed) {
                showNotification("Esta pauta está fechada. Ações não podem ser realizadas.", "error");
                return;
            }
      ...
      if (button.id === 'add-assisted-btn') {
        ...
        if (currentMode === 'agendamento') {
            ...
            if (isScheduled && !scheduledTimeValue) {
    ...
    // --- INÍCIO DA LÓGICA DE COLABORADORES (CORRIGIDO COM FIRESTORE) ---
    ...
    const collaboratorListTableBody = document.querySelector('#collaborators-list-table tbody');
    const addCollaboratorBtn = document.getElementById('add-collaborator-btn');
    const downloadCollaboratorsPdfBtn = document.getElementById('download-collaborators-pdf');
    const clearCollaboratorsListBtn = document.getElementById('clear-collaborators-list');
    ...
    const setupCollaboratorsListener = (pautaId) => {
        if (!auth.currentUser) {
            console.error("Tentativa de iniciar o listener de colaboradores sem usuário autenticado.");
            showNotification("Erro de autenticação. Por favor, recarregue a página.", "error");
            return;
        }
        ...
        unsubscribeFromCollaborators = onSnapshot(collaboratorsCollectionRef, (snapshot) => {
            ...
            collaboratorListTableBody.innerHTML = '<tr><td colspan="6" class="text-center py-4">Erro ao carregar colaboradores.</td></tr>';
        });
    };

    const renderColaboradores = () => {
        collaboratorListTableBody.innerHTML = '';

        let selfTransport = 0;
        let companyTransport = 0;
        
        if (colaboradores.length === 0) {
            collaboratorListTableBody.innerHTML = '<tr><td colspan="6" class="text-center py-4 text-gray-500">Nenhum colaborador adicionado.</td></tr>';
        ...
        colaboradores.forEach((colaborador) => {
            ...
            newRow.innerHTML = `
                ...
                <td class="py-4 px-6">
                    <a href="https://wa.me/55${colaborador.telefone.replace(/\D/g, '')}?text=${encodeURIComponent(`Bom dia! Poderia, por gentileza, informar se já está chegando? Estamos quase saindo.`)}" class="text-blue-600 hover:underline" target="_blank">Mensagem</a>
                </td>
            ...
            `;
            collaboratorListTableBody.appendChild(newRow);
        });
        ...
        document.querySelectorAll('.checkin-checkbox').forEach(checkbox => {
            checkbox.addEventListener('change', async (e) => {
                const collaboratorId = e.target.dataset.id;
                const collaboratorRef = doc(db, "pautas", currentPautaId, "collaborators", collaboratorId);
                if (e.target.checked) {
                    const now = new Date();
        ...
        document.querySelectorAll('.edit-collaborator-btn').forEach(button => {
            button.addEventListener('click', (e) => {
                const collaboratorId = e.currentTarget.dataset.id;
                editCollaboratorId = collaboratorId;
                const colaborador = colaboradores.find(c => c.id === collaboratorId);
                if (!colaborador) return;
                
                document.getElementById('collaborator-name').value = colaborador.nome;
                document.getElementById('collaborator-phone').value = colaborador.telefone;
                document.getElementById('collaborator-obs').value = colaborador.observacao || '';

                const transporteRadio = document.querySelector(`input[name="transporte"][value="${colaborador.transporte}"]`);
                if (transporteRadio) {
                    transporteRadio.checked = true;
                    transporteRadio.dispatchEvent(new Event('change'));
                }

                const localEncontroRadio = document.querySelector(`input[name="localEncontro"][value="${colaborador.localEncontro}"]`);
                if (localEncontroRadio) {
                    localEncontroRadio.checked = true;
                    localEncontroRadio.dispatchEvent(new Event('change'));
                }

                addCollaboratorBtn.textContent = "Salvar Alterações";
            });
        });

        document.querySelectorAll('.delete-collaborator-btn').forEach(button => {
            button.addEventListener('click', async (e) => {
                const collaboratorId = e.currentTarget.dataset.id;
                const colaborador = colaboradores.find(c => c.id === collaboratorId);
                if (colaborador && confirm(`Tem certeza que deseja excluir o colaborador "${colaborador.nome}"?`)) {
                    await deleteDoc(doc(db, "pautas", currentPautaId, "collaborators", collaboratorId));
                    showNotification("Colaborador excluído.", "success");
                }
            });
        });
    };
    ...
    const resetCollaboratorForm = () => {
        ...
        document.querySelector('input[name="transporte"][value="Não Confirmado"]').checked = true;
        editCollaboratorId = null;
        addCollaboratorBtn.textContent = "Adicionar Colaborador";
    ...
    transporteOptions.forEach(option => {
        option.addEventListener('change', (event) => {
            localEncontroGroup.classList.toggle('hidden', event.target.value !== 'Com a Empresa');
            observacaoGroup.classList.add('hidden'); 
            if (event.target.value !== 'Com a Empresa') {
                document.querySelectorAll('input[name="localEncontro"]').forEach(localOption => localOption.checked = false);
            }
        });
    });

    document.querySelectorAll('input[name="localEncontro"]').forEach(option => {
        option.addEventListener('change', (event) => {
            observacaoGroup.classList.toggle('hidden', event.target.value !== 'Meio do Caminho');
        });
    });

    collaboratorForm.addEventListener('submit', async (event) => {
        event.preventDefault();
        const nome = document.getElementById('collaborator-name').value;
        const telefone = document.getElementById('collaborator-phone').value;
        const transporte = collaboratorForm.querySelector('input[name="transporte"]:checked').value;
        const localEncontro = collaboratorForm.querySelector('input[name="localEncontro"]:checked')?.value || '';
        const observacao = document.getElementById('collaborator-obs').value;

        const collaboratorData = { nome, telefone, transporte, localEncontro, observacao };

        try {
            if (editCollaboratorId) {
                const collaboratorRef = doc(db, "pautas", currentPautaId, "collaborators", editCollaboratorId);
    ...
    downloadCollaboratorsPdfBtn.addEventListener('click', () => {
        const { jsPDF } = window.jspdf;
        const docPDF = new jsPDF();
        const head = [['Nome', 'Deslocamento', 'Presença', 'Horário de Chegada']];
        const tableBody = [];
        const presentes = colaboradores.filter(c => c.presente);

        if (presentes.length === 0) {
            showNotification("Nenhum colaborador com presença marcada.", "info");
            return;
        }
        ...
        presentes.forEach(colaborador => {
            ...
            tableBody.push([colaborador.nome, deslocamentoTexto, 'Presente', colaborador.horario]);
        });

        docPDF.autoTable({
            head: head, body: tableBody, startY: 20,
            headStyles: { fillColor: [34, 197, 94] },
            didDrawPage: (data) => {
                docPDF.setFontSize(18);
                docPDF.setTextColor(40);
                docPDF.text(`Lista de Presença - ${document.getElementById('pauta-title').textContent}`, data.settings.margin.left, 15);
            }
        });
        docPDF.save('lista_presenca_colaboradores.pdf');
    });

    clearCollaboratorsListBtn.addEventListener('click', async () => {
        if (confirm('Tem certeza que deseja limpar toda a lista? Esta ação não pode ser desfeita.')) {
            ...
            if (snapshot.empty) {
                showNotification("A lista já está limpa.", "info");
                return;
            }
            const batch = writeBatch(db);
            snapshot.docs.forEach(doc => batch.delete(doc.ref));
    ...
    document.getElementById('manage-collaborators-btn').addEventListener('click', () => {
        setupCollaboratorsListener(currentPautaId);
        resetCollaboratorForm();
        collaboratorsModal.classList.remove('hidden');
    });

    document.getElementById('close-collaborators-modal-btn').addEventListener('click', () => {
        if (unsubscribeFromCollaborators) {
            unsubscribeFromCollaborators();
        }
        collaboratorsModal.classList.add('hidden');
    });
    // --- FIM DA LÓGICA DE COLABORADORES ---
});
